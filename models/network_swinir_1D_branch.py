...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        self.conv_first1 = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
        self.conv_first2 = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
        #####################################################################################################
        ...
        self.patch_embed2 = PatchEmbed( # for the second branch
            img_size=img_size, patch_size=patch_size, in_chans=embed_dim, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
        ...
        print("Here is the true start!!!", len(dpr))
        ...
        if resi_connection == '1conv':
            ...
            # to save parameters and memory
            self.conv_after_body1 = nn.Sequential(nn.Conv2d(embed_dim, embed_dim // 4, 3, 1, 1),
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 nn.Conv2d(embed_dim // 4, embed_dim // 4, 1, 1, 0),
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 nn.Conv2d(embed_dim // 4, embed_dim, 3, 1, 1))

            self.conv_after_body2 = nn.Sequential(nn.Conv2d(embed_dim, embed_dim // 4, 3, 1, 1),
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 nn.Conv2d(embed_dim // 4, embed_dim // 4, 1, 1, 0),
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 nn.Conv2d(embed_dim // 4, embed_dim, 3, 1, 1))                                                 
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            print(num_feat, num_out_ch)
            self.conv_last = nn.Conv2d(num_feat*2, num_out_ch, 3, 1, 1)
        else:
            self.conv_last = nn.Conv2d(embed_dim*2, num_out_ch*2, 3, 1, 1)
    ...
    #reformat input from x to in_img
    # def forward(self, x):
    def forward(self, in_data):
        # x,y = torch.split(in_data, [3, 3], dim=1)      
        device = in_data.device
        x = torch.index_select(in_data, 1, torch.tensor([0, 3], device=device)) #x = torch.index_select(tensor, 1, torch.tensor([0, 3]))
        y = torch.index_select(in_data, 1, torch.tensor([1, 2], device=device)) #[16, 400, 2], [16, 400, 6] -> wave, loc
        H, W = x.shape[2:]
        x = self.check_image_size(x)
        y = self.check_image_size(y)
        # print("Here first!", x.shape, y.shape)
        # sys.exit()          
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            print(f'conv_first x:{x.shape}')   
            x = self.forward_features1(x)
            print(f'forward_features x:{x.shape}')
            x = self.conv_after_body(x) + x
            print(f'conv_after_body x:{x.shape}')
            x = self.conv_before_upsample(x)
            print(f'conv_before_upsample x:{x.shape}') 
            x = self.upsample(x)
            print(f'upsample x:{x.shape}')  
            print(f'in y:{y.shape}') 
            y = self.conv_first(y)
            print(f'conv_first y:{y.shape}')   
            y = self.forward_features2(y)
            print(f'forward_features y:{y.shape}')
            y = self.conv_after_body(y) + y
            print(f'conv_after_body y:{y.shape}')
            y = self.conv_before_upsample(y)
            print(f'conv_before_upsample y:{y.shape}') 
            y = self.upsample(y)
            print(f'upsample y:{y.shape}')             
            x = torch.cat((x, y), dim=1)
            print(f'concate dimension:{x.shape}')
            #=== Branch of two features
            x = self.conv_last(x)
            print(f'conv_last x:{x.shape}')
        else:
            # print(f'in x:{x.shape}')
            x_first = self.conv_first1(x)
            # print(f'conv_first x:{x.shape}') 
            x_forw = self.forward_features1(x_first) #forward_features1
            # print(f'forward_features x:{x_forw.shape}')
            res_x = self.conv_after_body1(x_forw) + x_first
            # print(f'conv_after_body x:{res_x.shape}')

            # print(f'in y:{y.shape}')
            y_first = self.conv_first2(y)
            # print(f'conv_first y:{y.shape}')
            y_forw = self.forward_features2(y_first) #forward_features1
            # print(f'forward_features y:{y_forw.shape}')
            res_y = self.conv_after_body2(y_forw) + y_first
            # print(f'conv_after_body y:{res_y.shape}')

            #==== CAT ===#
            new_res = torch.cat((res_x, res_y), dim=1)
            new_x = torch.cat([x[:, 0:1], y[:, 0:1], y[:, 1:2], x[:, 1:2]], dim=1)
            # print(f'concate dimension new_x; new_res:{new_x.shape}, {new_res.shape}')
            # print("conv_last: ", self.conv_last(new_res).shape)
            x = new_x + self.conv_last(new_res)
...
if __name__ == '__main__':
    ...
    model = SwinIR(upscale=2, img_size=(height, width),
                   window_size=window_size, img_range=1., depths=[6, 6, 6, 6],
                   in_chans=2, embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='')                   
    # print(model)
    # print(height, width, model.flops() / 1e9)

    x = torch.randn((7, 4, height, width))
