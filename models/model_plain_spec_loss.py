...
class ModelPlain(ModelBase):
    ...
    # define loss
    # ----------------------------------------
    def define_loss(self, in_mask=False):
        ...
        elif G_lossfn_type == 'WeightedL2Loss':
            self.G_lossfn = WeightedL2Loss().to(self.device)            
        else:
    ...
    def feed_data(self, data, need_H=True):
        ...
        self.mask = data['masks'].to(self.device)
        if need_H:
            self.H = data['H'].to(self.device)
        # print(self.mask.shape, self.L.shape, self.H.shape)

    def feed_data_test(self, data, need_H=True):
        self.L = data['L'].to(self.device)
        # self.mask = data['masks'].to(self.device)
        if need_H:
            self.H = data['H'].to(self.device)
    ...
    def netG_forward(self):
        try:
            # self.print_memory_usage()  # Print memory usage before forward pass
            # self.E = self.netG(self.L)
            self.E = self.netG(self.L)
            # self.mask = masks
            # print("successfully return self.E!!")
            # sys.exit()         
    ...
    def optimize_parameters(self, current_step):
        ...
        self.netG_forward()
        # print(type(self.E), self.E.shape, type(self.H), self.H.shape)
        # sys.exit()
        G_loss = self.G_lossfn_weight * self.G_lossfn(self.E * self.mask, self.H * self.mask)
        # G_loss = F.mse_loss(self.H * self.mask, self.E * self.mask, reduction='sum') / self.E.sum()
        # print("finish loss calculation")
        G_loss.backward()
